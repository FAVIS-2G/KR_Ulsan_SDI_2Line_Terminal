...
Public Function PWM_Init() As Boolean
On Error GoTo ErrorHandle
    
    pwm_BoardCount = OpenDAQDevice() + 1
    If pwm_BoardCount <= 0 Then
        PWM_Init = False
        Exit Function
    End If
...
Public Function PWM_Close() As Boolean

    PWM_Close = CloseDAQDevice
    
End Function

Public Sub PWM_SetLight(CH As Long, Value As Long)

    If CH < 0 Then
        Exit Sub
    End If
    
    If CH >= pwm_BoardCount * 4 Then
        Exit Sub
    End If
    ...
    Call Set_Pwm_Mul(nBoard, nChannel, Value)

End Sub

Public Sub PWM_SetLightAll(Value As Long)
    ...
    For i = 0 To CH - 1
        Call PWM_SetLight(i, Value)
    Next i
    
End Sub

Public Sub PWM_Light(CH As Integer, OnOff As Boolean, Optional Delay As Long = 0)
    ...
    If OnOff = True Then
        Call Pwm_Enable_Mul(nBoard, nChannel)
    Else
        Call Pwm_Disable_Mul(nBoard, nChannel)
    End If
...
Public Sub PWM_LightOn(CH As Long, Optional Delay As Long = 0)
...
Public Sub PWM_LightOff(CH As Long)
...
Public Sub PWM_LightAll(OnOff As Boolean, Optional Delay As Long = 0)
