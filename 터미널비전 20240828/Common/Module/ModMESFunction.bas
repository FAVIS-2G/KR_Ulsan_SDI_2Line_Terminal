...
Public Sub MES_ServerOpen()
    ...
    frmMain.WinsockMES.LocalPort = CInt(frmMain.txtMESPort.Text)
...
Dim i As Integer
    ...
    tmpStrDefaultMsg = tmpStrDefaultMsg & "      <FROM>" & ms_Source & "</FROM>" & vbCrLf
    tmpStrDefaultMsg = tmpStrDefaultMsg & "      <TO>" & ms_Destination & "</TO>" & vbCrLf
    tmpStrDefaultMsg = tmpStrDefaultMsg & "      <RECIPE_ID>" & sNowRecipeID & "</RECIPE_ID>" & vbCrLf
    ...
    Select Case msgid
        ...
        Case "EQ_STATE_EVENT"
            ...
            'AUTO
            If pItem = "AUTO" Then
                tmpStrDataMsg = tmpStrDataMsg & Space(6) & "<READY></READY>" & vbCrLf
            ...
            ElseIf pItem = "MANUAL" Then    '수동 변경 시 메인트 처리필요
                tmpStrDataMsg = tmpStrDataMsg & Space(6) & "<READY></READY>" & vbCrLf
                tmpStrDataMsg = tmpStrDataMsg & Space(6) & "<STATE>MANUAL</STATE>" & vbCrLf
                tmpStrDataMsg = tmpStrDataMsg & Space(6) & "<PROCESS_STATE></PROCESS_STATE>" & vbCrLf
                tmpStrDataMsg = tmpStrDataMsg & Space(6) & "<MAINT_STATE></MAINT_STATE>" & vbCrLf
                tmpStrDataMsg = tmpStrDataMsg & Space(6) & "<MAINT_CODE></MAINT_CODE>" & vbCrLf
            End If
        ...
        Case "NG_PRODUCT_EVENT"
            tmpS_IDcode = pItem
            If tmpS_IDcode = "NOID" Then
                tmpS_IDcode = ""
            End If
            ' MES 담당자 요청으로 loss count를 무조건 1 내보냄
            'tmpStrDataMsg = tmpStrDataMsg & Space(6) & "     <LOSS_COUNT>" & CStr(bResultjudge_cnt + iResultJudge_BlobCnt) & "</LOSS_COUNT>" & vbCrLf
            tmpStrDataMsg = tmpStrDataMsg & Space(6) & "     <LOSS_COUNT>" & CStr(1) & "</LOSS_COUNT>" & vbCrLf
'            For tooln = 0 To (iToolCount / 2) - 1
'                If bResultJudge_Spec(0, tooln) = False Then
                    tmpStrDataMsg = tmpStrDataMsg & Space(6) & "     <LOSS_DATA>" & vbCrLf
                    tmpStrDataMsg = tmpStrDataMsg & Space(6) & "          <LOT_ID>" & tmpS_IDcode & "</LOT_ID>" & vbCrLf
'                    tmpStrDataMsg = tmpStrDataMsg & Space(6) & "          <LOSS_CD>" & sParamName_NG(iNowRecipeID, tooln + 1) & "</LOSS_CD>" & vbCrLf
                    tmpStrDataMsg = tmpStrDataMsg & Space(6) & "          <LOSS_CD>" & sParamName_NG(iNowRecipeID, 1) & "</LOSS_CD>" & vbCrLf
                    tmpStrDataMsg = tmpStrDataMsg & Space(6) & "     </LOSS_DATA>" & vbCrLf
'                End If
'            Next tooln
...
'                tmpStrDataMsg = tmpStrDataMsg & Space(6) & "     <LOSS_DATA>" & vbCrLf
'                tmpStrDataMsg = tmpStrDataMsg & Space(6) & "          <LOT_ID>" & tmpS_IDcode & "</LOT_ID>" & vbCrLf
'                tmpStrDataMsg = tmpStrDataMsg & Space(6) & "          <LOSS_CD>" & 4310 & "</LOSS_CD>" & vbCrLf
'                tmpStrDataMsg = tmpStrDataMsg & Space(6) & "     </LOSS_DATA>" & vbCrLf
'            End If
            bResultjudge_cnt = 0
        Case "QMS_EVENT"
            ...
            tmpStrDataMsg = tmpStrDataMsg & Space(9) & "<PARAM>" & vbCrLf
            tmpStrDataMsg = tmpStrDataMsg & Space(12) & "<SLOT_ID></SLOT_ID> & vbCrLf"
            tmpStrDataMsg = tmpStrDataMsg & Space(12) & "<PARAM_NAME></PARAM_NAME> & vbCrLf"
            tmpStrDataMsg = tmpStrDataMsg & Space(12) & "<PARAM_VALUE></PARAM_VALUE> & vbCrLf"
            ...
            tmpStrDataMsg = tmpStrDataMsg & Space(12) & "<PARAM_NAME>" & sParamName_PV(iNowRecipeID, i) & "</PARAM_NAME>" & vbCrLf
            tmpStrDataMsg = tmpStrDataMsg & Space(12) & "<PARAM_VALUE>" & dScore & "</PARAM_VALUE>" & vbCrLf
            tmpStrDataMsg = tmpStrDataMsg & Space(12) & "<STEP_NAME></STEP_NAME>" & vbCrLf
            tmpStrDataMsg = tmpStrDataMsg & Space(12) & "<STEP_CONDITION></STEP_CONDITION>" & vbCrLf
            tmpStrDataMsg = tmpStrDataMsg & Space(9) & "</PARAM>" & vbCrLf
            tmpStrDataMsg = tmpStrDataMsg & Space(6) & "</PV>" & vbCrLf
        Case "LINKTEST_EVENT"
            tmpStrDataMsg = tmpStrDataMsg
        Case Else
...
Dim sVBTab As String
    ...
    For i = 0 To (iToolCount / 2) - 1
        ...
        End If
        tempStrSV = tempStrSV & sParamName_SVsj & lstr_tempSVcode & "=" & dSpecOri(i) & vbCrLf
        ...
        End If
        tempStrSV = tempStrSV & sParamName_SVsj & lstr_tempSVcode & "=" & dSpecMax(i) & vbCrLf
        ...
        End If
        tempStrSV = tempStrSV & sParamName_SVsj & lstr_tempSVcode & "=" & dSpecMin(i) & vbCrLf
        
    Next i
    
    For i = 0 To (iToolCount / 2) - 1
        tempStrPV = tempStrPV & sParamName_PV(iNowRecipeID, i + 1) & "=" & dInspectResult_mm(Index, i) & vbCrLf
    Next i
...
End Function
Public Sub MES_DATASEND_FUNC(msgid As String, msgid2 As String, ReplySys As String)
Dim tempStrMsg As String
    If ReplySys = "" Then
        'iMesSysbyte = iMesSysbyte + 1
    End If
